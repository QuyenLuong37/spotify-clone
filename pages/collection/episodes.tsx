      ...
      <div>
        ...
        <div className='px-6'>
            {savedEpisode?.items?.map((item, index) => {
                return (
                    <EpisodeItem key={index} description={item.description} duration_ms={item.duration_ms} image={item.images[0].url} name={item.name} release_date={item.release_date} uri={item.uri} showName={item.show.name} />
                )
            })}
        </div>
      </div>
